...
class Generator:
    ...
    def count_busy_neighbours(self, pos: tuple) -> int:
        return self.count(point for point in self.field.get_environment(*pos)
                          if not self.field.is_valid(*point) or
                          self.field[point] != CELL_EMPTY_VALUE)
    ...
    def is_isolated(self, pos: tuple, number: int, is_last: bool) -> bool:
        return (self.count_busy_neighbours(pos) == COUNT_NEIGHBORS and
                (not is_last or self.is_cycle(pos, number)))
    ...
    def has_isolated_empty_cells(self, pos: tuple,
        ...
        return any(
            self.is_isolated(point, number, is_last) for point in neighbours
            if self.field[point] == CELL_EMPTY_VALUE)
    ...
    def get_path_extens_neighbours(self, pos: tuple, number: int) -> tuple:
        ...
        if not self.has_isolated_empty_cells(pos, number, False):
            for pos in neighbours[idx:] + neighbours[:idx]:
                if (self.field[pos] == CELL_EMPTY_VALUE and
                        self.can_add_cell(pos, number)):
                    return pos

    def try_get_path_start(self) -> Tuple[tuple, tuple]:
    ...
    def add_path(self, start: tuple, end: tuple):
        ...
        while True:
            ...
            if (end is not None and
                    self.covered_cells < self.cells_amount):
    ...
    def create(self) -> List[List]:
        while True:
            ...
            if pair is not None:
                self.add_path(*pair)
            elif (self.covered_cells >= self.cells_amount and
